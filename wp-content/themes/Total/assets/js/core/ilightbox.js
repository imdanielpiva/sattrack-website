...
(function($, window, undefined) {
	...
	iLightBox.prototype = {
		...
		generateBoxes: function() {
			...
			if (opts.infinite && vars.total >= 2) { // WPEXPLORER - modified changed 3 to 2
	...
	function doAjax(url, callback) {
		var url = "https://ilightbox.net/getSource/jsonp.php?url=" + encodeURIComponent(url).replace(/!/g, '%21').replace(/'/g, '%27').replace(/\(/g, '%28').replace(/\)/g, '%29').replace(/\*/g, '%2A');
		$.ajax({
			url: url,
			dataType: 'jsonp'
		});
	...
	function getExtension(URL) {
		...
		ext = ext.replace(extra, ''); // EXTRA FIX BY WPEXPLORER
		return ext.replace('?', '');
	}
