...
((function () {
	...
	const bodyContent = $('#bd');

	const divFather = $('<div></div>').attr('id', 'modal1').addClass('modal modal-fixed-footer');

	const divContent = $('<div></div>').addClass('modal-content');

	const divHeader = $('<div></div>').addClass('row center');
	...
	const divFooter = $('<div></div>').addClass('modal-footer');

	const contentFooter = $('<a>Fechar</a>').addClass('modal-close waves-effect waves-light btn left');

	divFather.appendTo(bodyContent);
